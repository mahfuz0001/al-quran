...
const HomePage = () => {
  ...
  useEffect(() => {
    if (selectedSurah !== undefined && selectedAyat && selectedQuari) {
      ...
      const url = `http://api.alquran.cloud/v1/surah/1`;
      // console.log(url);
  ...
  const playPrev = () => {
    if (ayatDetails.prev_surah !== selectedSurah.id) {
      const newSelectedSurah = surahs.find(
        (surah) => surah.id === ayatDetails.prev_surah
      );
  ...
  const playNext = () => {
    if (ayatDetails.next_surah !== selectedSurah.id) {
      const newSelectedSurah = surahs.find(
        (surah) => surah.id === ayatDetails.next_surah
      );
      setSelectedSurah(newSelectedSurah);
      setSelectedAyat(1);
    } else {
    ...
    <div className="min-h-screen bg-blend-overlay bg-mainBg bg-cover bg-no-repeat opacity-90 bg-gray-900 py-5 px-2 md:px-10 flex flex-col justify-between">
      ...
      <main className="flex justify-between items-center px-1 md:px-5 gap-x-2 gap-y-8 py-3 md:py-2">
        <div>
          <div
            onClick={playPrev}
            className="w-[30px] h-[30px] rounded-full bg-gray-800 flex justify-center items-center"
          >
            <AiOutlineArrowLeft
              color="white"
              size={20}
        ...
        {!loading ? (
          <div className="p-1 md:p-5">
            <h2
              className={`text-center text-md md:text-4xl text-white ${
                !arabicFont ? `font-noorEHidayat` : `font-${fonts[arabicFont]}`
              }`}
            >
      ...
      <div>
        <div className="w-full flex flex-wrap gap-y-5 justify-center items-center gap-x-4">
          <div>
            <select
              onChange={quariHandler}
              className="select w-full select-bordered bg-gray-800 text-white"
            >
              {quari && quari.length > 0 ? (
                quari.map((quari, i) => (
                  <option value={quari.id} key={quari.id}>
                    {i + 1}. {quari.name}
                  </option>
                ))
              ) : (
        ...
        <div className="mt-3">
          <h2 className="text-center md:text-end text-white text-base">
            Build With ❤ By <a href="#">Mahfuz</a>
          </h2>
        </div>
      </div>
    </div>
